<script lang="ts">
    ...
    let color1: string = $derived(c1 + "df");
...
<div
    ...
    class="{switching
        ...
        : animations[animation].in} solid border{borderRadiusPx === 0 ? "-bottom" : ""}"
...
<style>
    ...
    .border {
        border: calc(var(--stroke-width) / 2) solid var(--stroke);
    }

    .border-bottom {
        border-bottom: calc(var(--stroke-width) / 2) solid var(--stroke);
    }
